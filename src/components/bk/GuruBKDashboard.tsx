...
import {
  ...
  BookOpen, Award, FileText, Plus, HeartHandshake, FileSpreadsheet, RefreshCw, Database
} from 'lucide-react';
...
export const GuruBKDashboard: React.FC<GuruBKDashboardProps> = ({ currentUser }) => {
  ...
  const getStudentTotalLogsCount = React.useCallback((student: User): number => {
    if (!student) return 0;
    const sId = student.id ? student.id.trim().toLowerCase() : '';
    const sUsername = student.username ? student.username.trim().toLowerCase() : '';
    const sName = student.name ? student.name.trim().toLowerCase() : '';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 rounded-2xl p-6 sm:p-8 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 border border-purple-800">
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0">
          <div className="flex items-center gap-3 bg-slate-800/90 p-2.5 rounded-xl border border-slate-700">
            <span className="text-xs font-bold text-slate-300">Tanggal:</span>
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="bg-slate-900 text-white text-xs font-bold px-3 py-1 rounded-lg border border-slate-600 outline-none"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-300 mt-0.5">
              Waktu Terakhir Disinkronkan: <strong className="text-white font-mono">{lastSyncText}</strong> • Total <strong className="text-purple-300">{allLogs.length} dokumen log</strong> tercatat dari siswa.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={isSyncing}
          className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl shadow flex items-center gap-2 transition-all disabled:opacity-50 shrink-0"
        >
            ...
            <tbody className="divide-y divide-slate-100">
              {paginatedStudents.length === 0 ? (
                ...
                paginatedStudents.map((student, idx) => {
                  ...
                  const totalStudentLogs = getStudentTotalLogsCount(student);
                  ...
                  return (
                    <tr key={student.id} className={`hover:bg-purple-50/50 transition-colors ${isLow ? 'bg-rose-50/30' : ''}`}>
                      ...
                      <td className="py-4 px-6 font-bold text-slate-900">
                        ...
                        <div className="mt-1 inline-flex items-center gap-1 px-2 py-0.5 bg-purple-50 text-purple-700 border border-purple-200 rounded text-[10px] font-bold">

        ...
        <div className="md:col-span-5 bg-gradient-to-br from-blue-900 via-indigo-900 to-slate-900 p-8 text-white flex flex-col justify-between relative overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-3.5 mb-6">
              <SchoolLogo size="xl" className="drop-shadow-[0_8px_16px_rgba(37,99,235,0.4)] shrink-0" />
              <div>
        ...
        <div className="md:col-span-7 p-6 sm:p-10 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-xl font-bold text-slate-900">Selamat Datang</h3>
                <p className="text-xs text-slate-500">Silakan masuk sesuai akun Anda</p>
              </div>
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl text-xs font-semibold">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'siswa' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  Siswa
                </button>
                <button
                  onClick={() => setActiveTab('guru')}
                  className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'guru' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  Guru / BK
                </button>
                <button
                  onClick={() => setActiveTab('admin')}
                  className={`px-3 py-1.5 rounded-lg transition-all ${activeTab === 'admin' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                >
                  Admin
                </button>
              </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  {activeTab === 'siswa' ? 'Nama Lengkap Siswa' : 'Username / Kelas'}
                </label>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200">
            <h4 className="text-xs font-bold text-slate-700 mb-2 flex items-center gap-1.5">
              ...
              <span>Akses Cepat Pengujian (1-Klik Login):</span>
            </h4>
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {/* Wali Kelas 8B */}
              {users.filter(u => u.role === 'wali_kelas' && u.assignedClass === '8B').map((u) => (
                <button
                  key={u.id}
                  onClick={() => handleQuickLogin(u)}
                  className="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-800 text-[10px] font-bold rounded-lg transition-colors"
